...
def evaluate_tagger(model, ctx_emb_function, gemb_eval_function, batched_dev_data, evaluator, writer, global_step):
  predictions = None
  dev_loss = 0
...
def train_gemb(args):
  ...
  with Timer('Data loading'):
    ...
    batched_dev_data = data.get_gemb_development_data(batch_size=config.dev_batch_size) # data for NN
  ...
  while epoch < config.max_epochs:
    ...
    if epoch % config.checkpoint_every_x_epochs == 0:
      with Timer('Evaluation'):
        evaluate_tagger(model, ctx_emb_function, gemb_eval_function, batched_dev_data, evaluator, writer, global_step)
