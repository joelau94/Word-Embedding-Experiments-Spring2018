...
class Parser(BaseParser):
  ...
  def add_char_gemb_graph(self, graph, reuse=False):
    ...
    graph['oov_char_rep'] = oov_char_rep
    graph['oov_pos'] = oov_pos
    graph['seq_len'] = seq_len
    char_input = tf.nn.embedding_lookup(embedding, oov_char_rep)
    # seq = tf.one_hot(oov_char_rep, len(self.vocabs[0]._char_idx2str))
    ...
    inputs_dimshuffle = tf.transpose(graph['inputs'], [1,0,2])
    # ctx_reshaped = tf.reshape(ctx, [-1,dim2,dim3])
    with tf.variable_scope('char_gemb'):
      feat = tf.contrib.layers.fully_connected(inputs=ctx,
            num_outputs=len(self.vocabs[0]._str2idx),
            activation_fn=None,
            scope='char_gemb_fc',
