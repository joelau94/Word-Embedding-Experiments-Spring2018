...
class Network(Configurable):
  ...
  def __init__(self, model, *args, **kwargs):
    ...
    # kwargs['model_type'] = kwargs.pop('model_type', model.__model_type__)
    ...
    if model_type == "gemb":
      self._ops = self._gen_gemb_ops()
    elif model_type == "char":
      self._ops = self._gen_char_ops()
  ...
  def train_gemb_minibatches(self):
    ...
    return self._train_gemb_set.get_minibatches_gemb_train(self.train_batch_size,
                                          self.model.input_idxs,
                                          self.model.target_idxs)
  def train_char_minibatches(self):
    """"""
    
    return self._train_gemb_set.get_minibatches_char_train(self.train_batch_size,
                                          self.model.input_idxs,
                                          self.model.target_idxs)

  def valid_char_minibatches(self):
    """"""
    
    return self._valid_gemb_set.get_minibatches_char_train(1,
                                          self.model.input_idxs,
                                          self.model.target_idxs, shuffle = False)
  ...
  def train_char(self, sess):
    ...
    try:
      ...
      while total_train_iters < train_iters:
        #print("Iteration:", total_train_iters)
        for j, (feed_dict, oov_pos, _, oov_char_rep, maxTrainSeqLen) in enumerate(self.train_char_minibatches()):
          # exit()
          train_inputs = feed_dict[self._train_gemb_set.inputs] # useless
          train_targets = feed_dict[self._train_gemb_set.targets] # useless
          batchSize = len(train_inputs)
          train_seq_len = np.ones(batchSize) * maxTrainSeqLen
          print("BATCH SIZE: ", batchSize)
          print("OOV:", oov_char_rep)
          feed_dict.update({self.train_gemb_graph['oov_pos']: oov_pos})
          feed_dict.update({self.train_gemb_graph['oov_char_rep']: oov_char_rep})
          feed_dict.update({self.train_gemb_graph['seq_len']: train_seq_len})
          ...
          if total_train_iters == 1 or total_train_iters % validate_every == 0:
            ...
            n_valid_tokens = 0
            with open(os.path.join(self.save_dir, 'sanitycheck.txt'), 'w') as f:
              for k, (feed_dict, oov_pos, _, oov_char_rep, maxValidSeqLen) in enumerate(self.valid_char_minibatches()):
                inputs = feed_dict[self._valid_gemb_set.inputs]
                targets = feed_dict[self._valid_gemb_set.targets]
                start_time = time.time()
                batchSize = len(inputs)
                valid_seq_len = np.ones(batchSize) * maxValidSeqLen
                # get gemb
                feed_dict.update({self.valid_get_gemb_graph['oov_pos']: oov_pos})
                feed_dict.update({self.train_gemb_graph['oov_char_rep']: oov_char_rep})
                feed_dict.update({self.train_gemb_graph['seq_len']: train_seq_len})
  ...
  def _gen_char_ops(self):
    ...
    train_gemb_op = optimizer2.minimize(train_gemb_graph['gemb_loss'],
        var_list=[var for var in tf.global_variables() if 'char_gemb/char_gemb_fc' in var.op.name])
    ...
    self._model.add_char_gemb_graph(valid_get_gemb_graph)
    ...
    self._model.add_char_gemb_graph(test_get_gemb_graph)
...
if __name__ == '__main__':
  ...
  argparser.add_argument('--train-char', action='store_true')
  argparser.add_argument('--train-mix', action='store_true')
  argparser.add_argument('--test-char', action='store_true')
  argparser.add_argument('--test-gemb', action='store_true')
  argparser.add_argument('--test-mix', action='store_true')
  ...
  if args.train_gemb:
    cargs['model_type'] = "gemb"
  if args.train_char:
    cargs['model_type'] = "char"
  if args.train_mix:
    cargs['model_type'] = "mix"
  ...
  with tf.Session(config=config_proto) as sess:
    ...
    if not (args.test or args.test_gemb or args.matrix):
      ...
      elif args.train_char:
        network.train_char(sess)
      else:
        network.train(sess)
    elif args.matrix:
      saver = tf.train.Saver(var_list=network.save_vars)
