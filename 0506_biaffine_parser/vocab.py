        ...
        if len(line) == 10:
            if hasattr(self.conll_idx, '__iter__'):
              for idx in self.conll_idx:
                ...
                for ch in chars:
                  char_counts[ch] += 1
                # print("CHAR VOCAB DEBUG : ", line[idx])
            else:
              word = line[self.conll_idx]
              chars = list(word.lower())
              for ch in chars:
                char_counts[ch] += 1
              # print("CHAR VOCAB DEBUG CONLL IDX: ", line[self.conll_idx])
    self._char_counts = char_counts
    self._char_str2idx, self._char_idx2str = self.index_char_vocab(char_counts)
    return
  
  def index_char_vocab(self, counts):
    """"""
    
    char_str2idx = {}
    char_idx2str = {}
    cur_idx = 3
    li_chars = counts.most_common()
    char_str2idx['<UNK>'] = 0
    char_str2idx['<ROOT>'] = 1
    char_str2idx['<PAD>'] = 2
    char_idx2str[0] = "<UNK>"
    char_idx2str[1] = "<ROOT>"
    char_idx2str[2] = "<PAD>"
    for char, count in li_chars:
        char_str2idx[char] = cur_idx
        char_idx2str[cur_idx] = char
        cur_idx += 1
    return char_str2idx, char_idx2str
