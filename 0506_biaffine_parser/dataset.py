...
class Dataset(Configurable):
  ...
  def get_minibatches_char_train(self, batch_size, input_idxs, target_idxs, shuffle=True):
      """"""
      ...
      for bkt_idx, bkt_mb in minibatches:
        ...
        # print("1:", data_no_oov[:,:maxlen,input_idxs].shape)
        # print("2:", oov_pos.shape)
        # print("CHECK:", len(oov_pos) == len(data_no_oov[:,:maxlen,input_idxs]))
        # max(res['length'])

        seq_len = np.zeros([len(oov_pos)], dtype =np.int32)
        for i, datapoint in enumerate(data_no_oov[:,:maxlen,input_idxs]):
          oov_word = self.vocabs[0]._idx2str[datapoint[oov_pos[i], 0]]
          # li = []
          if oov_word in ["<ROOT>", "<PAD>", "<UNK>"]:
            seq_len[i] = 1
          else:
            seq_len[i] = len(oov_word)
        
        oov_char_rep = np.zeros([len(oov_pos), max(seq_len)], dtype=np.int32)
        
        for i, datapoint in enumerate(data_no_oov[:,:maxlen,input_idxs]):
          oov_word = self.vocabs[0]._idx2str[datapoint[oov_pos[i], 0]]
          li = []
          if oov_word in ["<ROOT>", "<PAD>", "<UNK>"]:
            li.append(self.vocabs[0]._char_str2idx[oov_word])
          else:
            for ch in oov_word:
              li.append(self.vocabs[0]._char_str2idx[ch])
          # print(tf.one_hot(li, len(self.vocabs[0]._char_idx2str)).shape)
          # x_i[:res['length'].values[i]] = res['as_numbers'].values[i]
          oov_char_rep[i, :len(li)] = np.array(li)
        # print(oov_char_rep)
        yield feed_dict, oov_pos, sents[no_oov], oov_char_rep, max(seq_len)
