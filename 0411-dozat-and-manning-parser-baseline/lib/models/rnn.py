...
def bidirectional_rnn(cell_fw, cell_bw, inputs, initial_state_fw=None, initial_state_bw=None, dtype=None, sequence_length=None, scope=None):
  ...
  outputs = [array_ops.concat(axis = 1, values = [fw, bw])
...
def dynamic_bidirectional_rnn(cell_fw, cell_bw, inputs, sequence_length, initial_state_fw=None, initial_state_bw=None, ff_keep_prob=1., recur_keep_prob=True, dtype=None, parallel_iterations=None, swap_memory=False, time_major=False, scope=None):
  ...
  # Concat each of the forward/backward outputs
  outputs = array_ops.concat(axis = 2, values = [output_fw, output_bw])
...
def _dynamic_rnn_loop( cell, inputs, initial_state, ff_keep_prob, recur_keep_prob, parallel_iterations, swap_memory, sequence_length=None):
  ...
  input_shape = array_ops.shape(inputs)
  (time_steps, batch_size, _) = array_ops.unstack(input_shape, 3)
  ...
  zero_output = array_ops.zeros(
      array_ops.stack([batch_size, cell.output_size]), inputs.dtype)
  if sequence_length is not None:
    min_sequence_length = math_ops.reduce_min(sequence_length)
    max_sequence_length = math_ops.reduce_max(sequence_length)
  ...
  if isinstance(ff_keep_prob, ops.Tensor) or ff_keep_prob < 1:
    inputs = nn_ops.dropout(inputs, ff_keep_prob, noise_shape=array_ops.stack([1, batch_size, const_depth]))
  input_ta = input_ta.unstack(inputs)
  
  if isinstance(recur_keep_prob, ops.Tensor) or recur_keep_prob < 1:
    ones = array_ops.ones(array_ops.stack([batch_size, cell.output_size]), inputs.dtype)
    state_dropout = nn_ops.dropout(ones, recur_keep_prob)
    state_dropout = array_ops.concat(axis = 1, values = [ones] * (cell.state_size // cell.output_size - 1) + [state_dropout])
  else:
    state_dropout = 1.
