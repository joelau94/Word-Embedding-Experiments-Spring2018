...
def get_srl_data(config, train_data_path, dev_data_path, vocab_path=None, label_path=None, with_raw=False):
  '''
  '''
  use_se_marker = config.use_se_marker
  ...
  if with_raw:
    return (train_sents, dev_sents, word_dict, label_dict,
        ...
        [raw_train_tokens, raw_dev_tokens])
  else:
    return (train_sents, dev_sents, word_dict, label_dict,
        [word_embedding, None, None],
        [word_embedding_shape] + feature_shapes,
...
def get_srl_test_data(filepath, config, word_dict, label_dict, allow_new_words=True):
  word_dict.accept_new = allow_new_words = False
  if label_dict.accept_new:
    label_dict.set_unknown_token(UNKNOWN_LABEL)
    label_dict.accept_new = False
...
def get_srl_test_data_gemb(filepath, config, word_dict, label_dict, allow_new_words=False, with_raw=False):
  ...
  # Get raw token
  raw_tokens = [sent[0] for sent in samples]
  ...
  if with_raw:
    return (sentences, [word_embedding, None, None], [word_embedding_shape,] + feature_shapes, raw_tokens)
  else:
