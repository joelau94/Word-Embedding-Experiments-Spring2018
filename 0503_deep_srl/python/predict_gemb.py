...
def get_scores_ctx(config, task, model_path, gemb_model_path, word_dict_path, label_dict_path, input_path):  
  with Timer('Data loading'):
    ...
    # Batching.
    test_data = data.get_ctx_gemb_test_data(test_sentences, batch_size=config.dev_batch_size)
...
def get_scores_mix(config, task, model_path, gemb_model_path, word_dict_path, label_dict_path, input_path):  
  with Timer('Data loading'):
    print ('Task: {}'.format(task))
    allow_new_words = False
    print ('Allow new words in test data: {}'.format(allow_new_words))
    ...
    label_dict.load(label_dict_path)
    data = TaggerData(config, [], [], word_dict, label_dict, None, None)
    data.load_char(args.model)

    # Load test data.
    if task == 'srl':
      test_sentences, emb_inits, emb_shapes, raw_sents = reader.get_srl_test_data_gemb(
                                                    input_path,
                                                    config,
                                                    data.word_dict,
                                                    data.label_dict,
    ...
    # Batching.
    test_data = data.get_mix_gemb_test_data(test_sentences, raw_sents, batch_size=config.dev_batch_size)
  ...
  with Timer('Running model'):
    scores = None
    for i, batched_tensor in enumerate(test_data):
      x, _, oov_pos, oov_char, rnn_mask, num_tokens, weights = batched_tensor # weights is mask
      oov_pos = oov_pos[0] # batch must be 1
...
def get_scores_char(config, task, model_path, gemb_model_path, word_dict_path, label_dict_path, input_path):  
  # use same data format as Mix
  with Timer('Data loading'):
    print ('Task: {}'.format(task))
    allow_new_words = False
    print ('Allow new words in test data: {}'.format(allow_new_words))
...
def get_scores(gemb_type, *args):
  if gemb_type == 'context':
    return get_scores_ctx(*args)
  elif gemb_type == 'char':
    return get_scores_char(*args)
  elif gemb_type == 'mix':
    return get_scores_mix(*args)
...
if __name__ == "__main__":
  ...
  for i in range(num_ensemble_models):
    ...
    scores, data, test_sentences, test_data = get_scores(args.gemb_type,
                                                         config,
                                                         args.task,
                                                         model_path,
                                                         gemb_model_path,
